...
const char *cl_error_to_str(cl_int e)
{
  switch (e)
  {
    ...
    case -3: return "device compiler not available";
...
void print_platforms_devices()
{
  ...
  cl_uint plat_count;
  { cl_int status_code; status_code = clGetPlatformIDs (0, ((void *)0), &plat_count); if ((status_code) != 0) { fprintf(stderr, "*** '%s' in '%s' on line %d failed with error '%s'.\n", "clGetPlatformIDs", "cl-helper.c", 113, cl_error_to_str(status_code)); abort(); }; };


  cl_platform_id *platforms =
    (cl_platform_id *) malloc(plat_count*sizeof(cl_platform_id));
  if (!platforms) { perror("allocating platform array"); abort(); };

  { cl_int status_code; status_code = clGetPlatformIDs (plat_count, platforms, ((void *)0)); if ((status_code) != 0) { fprintf(stderr, "*** '%s' in '%s' on line %d failed with error '%s'.\n", "clGetPlatformIDs", "cl-helper.c", 120, cl_error_to_str(status_code)); abort(); }; };
  ...
  for (cl_uint i = 0; i < plat_count; ++i)
  {
    ...
    char buf[1000];
    { cl_int status_code; status_code = clGetPlatformInfo (platforms[i], 0x0903, sizeof(buf), buf, ((void *)0)); if ((status_code) != 0) { fprintf(stderr, "*** '%s' in '%s' on line %d failed with error '%s'.\n", "clGetPlatformInfo",
 "cl-helper.c"
# 127 "cl-helper.c"
    ,
 128
# 127 "cl-helper.c"
    ...
    cl_uint dev_count;
    { cl_int status_code; status_code = clGetDeviceIDs (platforms[i], 0xFFFFFFFF, 0, ((void *)0), &dev_count); if ((status_code) != 0) { fprintf(stderr, "*** '%s' in '%s' on line %d failed with error '%s'.\n", "clGetDeviceIDs",
 "cl-helper.c"
# 133 "cl-helper.c"
    ,
 134
# 133 "cl-helper.c"
    ...
    if (!devices) { perror("allocating device array"); abort(); };


    { cl_int status_code; status_code = clGetDeviceIDs (platforms[i], 0xFFFFFFFF, dev_count, devices, ((void *)0)); if ((status_code) != 0) { fprintf(stderr, "*** '%s' in '%s' on line %d failed with error '%s'.\n", "clGetDeviceIDs",
 "cl-helper.c"
# 141 "cl-helper.c"
    ,
 142
# 141 "cl-helper.c"
    ...
    {
      char buf[1000];
      { cl_int status_code; status_code = clGetDeviceInfo (devices[j], 0x102B, sizeof(buf), buf, ((void *)0)); if ((status_code) != 0) { fprintf(stderr, "*** '%s' in '%s' on line %d failed with error '%s'.\n", "clGetDeviceInfo",
 "cl-helper.c"
# 148 "cl-helper.c"
      ,
 149
...
{
  char * line = (char *) malloc(1000), * linep = line;
...
{
  char dev_sel_buf[1000];
  char platform_sel_buf[1000];


  cl_uint plat_count;
  { cl_int status_code; status_code = clGetPlatformIDs (0, ((void *)0), &plat_count); if ((status_code) != 0) { fprintf(stderr, "*** '%s' in '%s' on line %d failed with error '%s'.\n", "clGetPlatformIDs", "cl-helper.c", 218, cl_error_to_str(status_code)); abort(); }; };


  cl_platform_id *platforms =
    (cl_platform_id *) malloc(plat_count*sizeof(cl_platform_id));
  if (!platforms) { perror("allocating platform array"); abort(); };
  { cl_int status_code; status_code = clGetPlatformIDs (plat_count, platforms, ((void *)0)); if ((status_code) != 0) { fprintf(stderr, "*** '%s' in '%s' on line %d failed with error '%s'.\n", "clGetPlatformIDs", "cl-helper.c", 224, cl_error_to_str(status_code)); abort(); }; };



  if (plat_name == CHOOSE_INTERACTIVELY)
...
# 235 "cl-helper.c"
    ...
    int sel_int = ((((((0)>(atoi(sel)))?(0):(atoi(sel))))<((int) plat_count-1))?((((0)>(atoi(sel)))?(0):(atoi(sel)))):((int) plat_count-1));
    free(sel);

    { cl_int status_code; status_code = clGetPlatformInfo (platforms[sel_int], 0x0903, sizeof(platform_sel_buf), platform_sel_buf, ((void *)0)); if ((status_code) != 0) { fprintf(stderr, "*** '%s' in '%s' on line %d failed with error '%s'.\n", "clGetPlatformInfo",
 "cl-helper.c"
# 253 "cl-helper.c"
    ,
 254
...
# 279 "cl-helper.c"
      ...
      if (dev_name == CHOOSE_INTERACTIVELY)
...
# 291 "cl-helper.c"
        ...
        int int_sel = ((((((0)>(atoi(sel)))?(0):(atoi(sel))))<((int) dev_count-1))?((((0)>(atoi(sel)))?(0):(atoi(sel)))):((int) dev_count-1));
        free(sel);

        { cl_int status_code; status_code = clGetDeviceInfo (devices[int_sel], 0x102B, sizeof(dev_sel_buf), dev_sel_buf, ((void *)0)); if ((status_code) != 0) { fprintf(stderr, "*** '%s' in '%s' on line %d failed with error '%s'.\n", "clGetDeviceInfo",
 "cl-helper.c"
# 309 "cl-helper.c"
        ,
 310
        ...
        if (!dev_name || strstr(buf, dev_name))
        {
          if (idx == 0)
          {
            ...
            cl_context_properties cps[3] = {
              0x1084, (cl_context_properties) plat, 0 };

            cl_int status;
            *ctx = clCreateContext(
                cps, 1, &dev, ((void *)0), ((void *)0), &status);
            if ((status) != 0) { fprintf(stderr, "*** '%s' in '%s' on line %d failed with error '%s'.\n", "clCreateContext", "cl-helper.c", 342, cl_error_to_str(status)); abort(); };
            ...
            if (queue)
            {
              *queue = clCreateCommandQueue(*ctx, dev, qprops, &status);
              if ((status) != 0) { fprintf(stderr, "*** '%s' in '%s' on line %d failed with error '%s'.\n", "clCreateCommandQueue", "cl-helper.c", 352, cl_error_to_str(status)); abort(); };
            }
...
char *read_file(const char *filename)
{
  ...
  if (!f) { perror("read_file: opening file"); abort(); };


  if (fseek(f, 0, 2) < 0) { perror("read_file: seeking to end"); abort(); };
  size_t size = ftell(f);

  if (fseek(f, 0, 0) != 0) { perror("read_file: seeking to start"); abort(); }
                                    ;


  char *result = (char *) malloc(size+1);
  if (!result) { perror("read_file: allocating file contents"); abort(); };
  if (fread(result, 1, size, f) < size) { perror("read_file: reading file contents"); abort(); }
                                         ;


  if (fclose(f)) { perror("read_file: closing file"); abort(); };
  result[size] = '\0';
    ...
    char const *knl, char const *knl_name, char const *options)
{
  ...
  if ((status) != 0) { fprintf(stderr, "*** '%s' in '%s' on line %d failed with error '%s'.\n", "clCreateProgramWithSource", "cl-helper.c", 419, cl_error_to_str(status)); abort(); };


  status = clBuildProgram(program, 0, ((void *)0), options, ((void *)0), ((void *)0));

  {


    cl_device_id dev;
    { cl_int status_code; status_code = clGetProgramInfo (program, 0x1163, sizeof(dev), &dev, ((void *)0)); if ((status_code) != 0) { fprintf(stderr, "*** '%s' in '%s' on line %d failed with error '%s'.\n", "clGetProgramInfo",
 "cl-helper.c"
# 428 "cl-helper.c"
    ,
 429
# 428 "cl-helper.c"
    ...
    size_t log_size;
    { cl_int status_code; status_code = clGetProgramBuildInfo (program, dev, 0x1183, 0, ((void *)0), &log_size); if ((status_code) != 0) { fprintf(stderr, "*** '%s' in '%s' on line %d failed with error '%s'.\n", "clGetProgramBuildInfo",
 "cl-helper.c"
# 432 "cl-helper.c"
    ,
 433
# 432 "cl-helper.c"
    ...
    if (!do_print && log_size)
    {
      ...
      else
      {
        if (!printed_compiler_output_message && !getenv("CL_HELPER_NO_COMPILER_OUTPUT_NAG"))
        {
          ...
          printed_compiler_output_message = 1;
        }
      }
    }
    ...
    if (do_print)
    {
      ...
      if (!log) { perror("kernel_from_string: allocate log"); abort(); };

      char devname[1000];
      { cl_int status_code; status_code = clGetDeviceInfo (dev, 0x102B, sizeof(devname), devname, ((void *)0)); if ((status_code) != 0) { fprintf(stderr, "*** '%s' in '%s' on line %d failed with error '%s'.\n", "clGetDeviceInfo",
 "cl-helper.c"
# 460 "cl-helper.c"
      ,
 461
# 460 "cl-helper.c"
      ...
      { cl_int status_code; status_code = clGetProgramBuildInfo (program, dev, 0x1183, log_size, log, ((void *)0)); if ((status_code) != 0) { fprintf(stderr, "*** '%s' in '%s' on line %d failed with error '%s'.\n", "clGetProgramBuildInfo",
 "cl-helper.c"
# 463 "cl-helper.c"
      ,
 464
# 463 "cl-helper.c"
  ...
  if ((status) != 0) { fprintf(stderr, "*** '%s' in '%s' on line %d failed with error '%s'.\n", "clBuildProgram", "cl-helper.c", 470, cl_error_to_str(status)); abort(); };


  cl_kernel kernel = clCreateKernel(program, knl_name, &status);
  if ((status) != 0) { fprintf(stderr, "*** '%s' in '%s' on line %d failed with error '%s'.\n", "clCreateKernel", "cl-helper.c", 474, cl_error_to_str(status)); abort(); };

  { cl_int status_code; status_code = clReleaseProgram (program); if ((status_code) != 0) { fprintf(stderr, "*** '%s' in '%s' on line %d failed with error '%s'.\n", "clReleaseProgram", "cl-helper.c", 476, cl_error_to_str(status_code)); abort(); }; };
...
# 538 "cl-helper.c"
  ...
  static struct { cl_device_info param; const char *name; } longProps[] = {
    ...
    { 0x1001, "VENDOR_ID" }, { 0x1002, "MAX_COMPUTE_UNITS" }, { 0x1003, "MAX_WORK_ITEM_DIMENSIONS" }, { 0x1004, "MAX_WORK_GROUP_SIZE" }, { 0x1006, "PREFERRED_VECTOR_WIDTH_CHAR" }, { 0x1007, "PREFERRED_VECTOR_WIDTH_SHORT" }, { 0x1008, "PREFERRED_VECTOR_WIDTH_INT" }, { 0x1009, "PREFERRED_VECTOR_WIDTH_LONG" }, { 0x100A, "PREFERRED_VECTOR_WIDTH_FLOAT" }, { 0x100B, "PREFERRED_VECTOR_WIDTH_DOUBLE" }, { 0x100C, "MAX_CLOCK_FREQUENCY" }, { 0x100D, "ADDRESS_BITS" }, { 0x1010, "MAX_MEM_ALLOC_SIZE" }, { 0x1016, "IMAGE_SUPPORT" }, { 0x100E, "MAX_READ_IMAGE_ARGS" }, { 0x100F, "MAX_WRITE_IMAGE_ARGS" }, { 0x1011, "IMAGE2D_MAX_WIDTH" }, { 0x1012, "IMAGE2D_MAX_HEIGHT" }, { 0x1013, "IMAGE3D_MAX_WIDTH" }, { 0x1014, "IMAGE3D_MAX_HEIGHT" }, { 0x1015, "IMAGE3D_MAX_DEPTH" }, { 0x1018, "MAX_SAMPLERS" }, { 0x1017, "MAX_PARAMETER_SIZE" }, { 0x1019, "MEM_BASE_ADDR_ALIGN" }, { 0x101A, "MIN_DATA_TYPE_ALIGN_SIZE" }, { 0x101D, "GLOBAL_MEM_CACHELINE_SIZE" }, { 0x101E, "GLOBAL_MEM_CACHE_SIZE" }, { 0x101F, "GLOBAL_MEM_SIZE" }, { 0x1020, "MAX_CONSTANT_BUFFER_SIZE" }, { 0x1021, "MAX_CONSTANT_ARGS" }, { 0x1023, "LOCAL_MEM_SIZE" }, { 0x1024, "ERROR_CORRECTION_SUPPORT" }, { 0x1025, "PROFILING_TIMER_RESOLUTION" }, { 0x1026, "ENDIAN_LITTLE" }, { 0x1027, "AVAILABLE" }, { 0x1028, "COMPILER_AVAILABLE" },

    { 0, ((void *)0) },
  };
  static struct { cl_device_info param; const char *name; } hexProps[] = {

    { 0x101B, "SINGLE_FP_CONFIG" }, { 0x102A, "QUEUE_PROPERTIES" },

    { 0, ((void *)0) },
  };
  static struct { cl_device_info param; const char *name; } strProps[] = {

    { 0x102B, "NAME" }, { 0x102C, "VENDOR" }, { 0x102E, "PROFILE" }, { 0x102F, "VERSION" }, { 0x1030, "EXTENSIONS" },

    { 0x102D, "DRIVER_VERSION" },
    { 0, ((void *)0) },
  };
  cl_int status;
  size_t size;
  ...
  for (ii = 0; strProps[ii].name != ((void *)0); ii++)
  {
  ...
  status = clGetDeviceInfo(device, 0x1000, sizeof val, &val, ((void *)0));
  if (status == 0)
  {
    printf("Type: ");
    if (val & (1 << 0))
  ...
  status = clGetDeviceInfo(device, 0x1029,
      sizeof val, &val, ((void *)0));
  if (status == 0)
  {
  ...
  status = clGetDeviceInfo(device, 0x101C,
      sizeof val, &val, ((void *)0));
  if (status == 0)
  {
  ...
  for (ii = 0; hexProps[ii].name != ((void *)0); ii++)
  {
  ...
  for (ii = 0; longProps[ii].name != ((void *)0); ii++)
  {
  ...
  {
    size_t size;
    { cl_int status_code; status_code = clGetDeviceInfo (device, 0x1005, 0, 0, &size); if ((status_code) != 0) { fprintf(stderr, "*** '%s' in '%s' on line %d failed with error '%s'.\n", "clGetDeviceInfo",
 "cl-helper.c"
# 716 "cl-helper.c"
    ,
 717
# 716 "cl-helper.c"
    ...
    { cl_int status_code; status_code = clGetDeviceInfo (device, 0x1005, size, res_vec, &size); if ((status_code) != 0) { fprintf(stderr, "*** '%s' in '%s' on line %d failed with error '%s'.\n", "clGetDeviceInfo",
 "cl-helper.c"
# 721 "cl-helper.c"
    ,
 722
...
{
  cl_device_id dev;
  { cl_int status_code; status_code = clGetCommandQueueInfo (queue, 0x1091, sizeof dev, &dev, ((void *)0)); if ((status_code) != 0) { fprintf(stderr, "*** '%s' in '%s' on line %d failed with error '%s'.\n", "clGetCommandQueueInfo",
 "cl-helper.c"
# 737 "cl-helper.c"
  ,
 738
